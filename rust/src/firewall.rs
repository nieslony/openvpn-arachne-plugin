...
#[path = "firewalld/firewall_d1.rs"] mod firewalld_firewall_d1;
...
fn create_rich_rule(
    ...
    format!(
        "rule family=\"ipv4\"{source_address}{destination_address}{service_name}{port}"
    )
}
...
pub fn firewalld_update_everybody_rules(
    ...
    match response.status() {
         StatusCode::OK => {
             client.note("User successfuly authenticated with token")
         },
         _ => {
    ...
    client.note(format!("{:?}", policy).as_str());
    let rich_rules = match &j["richRules"] {
        JsonValue::Array(a) => a,
        _ => {
            client.error("No richRules provided");
            return false
        }
    };
    for rule in rich_rules {
        client.note(format!("{:?}", rule).as_str());
        client.note(create_rich_rule(rule).as_str());
    }

    true
}

pub fn firewall_reload(
    handle: &mut Handle,
    client: &VpnClient
) -> bool {
    let firewall = match FirewallD1ProxyBlocking::new(
            &handle.dbus_connection.as_ref().unwrap()
    ) {
        Ok(f) => f,
        Err(msg) => {
